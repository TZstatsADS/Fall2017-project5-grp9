...
>>>>>>> 9172a0da06504d65871da4d8675376387de4c08b
###So where ya'll from? 
...
```{r}
ggplot(data)+geom_histogram(aes(meanfun),bins=60,fill="coral")
```
...
```{r}
ggplot(data)+geom_histogram(aes(meanfun,fill=sex),bins=60)+scale_fill_brewer(palette="Set3")
```
...
##Fix typos on countries
data$country<-gsub("kosovo","serbia",data$country)
data$country<-gsub("sicily","italy",data$country)
data$country<-gsub("tibet","china",data$country)
data$country<-gsub("yugoslavia","serbia",data$country)
data$country<-gsub("virginia","usa",data$country)
##One NA for Laos
data$country[is.na(data$country)]<-"laos"

data$regions<-countrycode((data$country),"country.name","region")
data$continent<-countrycode((data$country),"country.name","continent")
data$continent<-as.factor(data$continent)
data$regions<-as.factor(data$regions)
contdf<-data[,c(2:23,32)]

regiondf<-data[,c(2:23,31)]
#Recursive Feature Elimination
control<-trainControl(method="repeatedcv",number = 10, repeats = 3)
#genderdf[,-23]<-scale(genderdf[,-23])
#genderdf$sex<-droplevels(genderdf$sex) 
regiondf$regions<-as.factor(regiondf$regions)
model<-train(regions~.,data=regiondf,method="lvq", preProcess="scale", trControl=control)
...
results <- rfe(continent~.,data=bigcont, sizes=c(1:8), rfeControl=control)
